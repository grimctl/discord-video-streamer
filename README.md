...
- Starts with a larger IPTV-friendly playback cushion, keeps a best-effort steady buffer when possible, and re-buffers cleanly if the cushion runs low
...
- `stream.startupTimeoutMs`: maximum time allowed for probe, transcode startup, and initial buffer fill
- `stream.buffer.startupMs`: initial playback delay before Discord starts receiving media, default `20000`
...
- If the Discord gateway reconnects, the active stream is stopped, the app blocks new control mutations briefly, and it now attempts to rejoin voice and resume the previous stream automatically before giving up.
...
- The adaptive playback buffer now defaults to `20s` startup, aims for `25s` when the source allows it, and re-buffers below `5s` until `10s` is available again.
